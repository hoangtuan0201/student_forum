    ...
    <div class="card-body p-2 p-sm-3">
        <div class="media forum-item">
            <div class="media-body">
                <h6>
                    <a href="/student_forum/app/views/post/post_detail.php?id=<?= $post['post_id']; ?>" class="text-body font-weight-bold"><?= $post["title"]; ?></a>
                </h6>
                <p class="text-secondary">
                    <?= strip_tags(substr($post["content"], 0, 50)) . (strlen($post["content"]) > 50 ? '...' : ''); ?>
                </p>
                
                <p class="text-muted mt-2 small">   
                    <a href="#" class="font-weight-bold"><?= $post["username"]; ?></a> 
                    posted in <strong><?= $post["module_name"]; ?></strong>
                ...
                <a href="/student_forum/app/views/post/post_detail.php?id=<?= $post['post_id']; ?>" class="btn btn-sm btn-outline-primary">Read more</a>
                    ...
                    <?php if (isset($_SESSION["user_id"]) && $_SESSION["user_id"] == $post["user_id"]) : ?>
                        <a href="/student_forum/app/views/post/edit_post.php?id=<?= $post['post_id']; ?>" class="btn btn-sm btn-outline-primary">Edit</a>
